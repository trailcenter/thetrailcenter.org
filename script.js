...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function renderCart() {
    ...
    if (!cart.length) {
      cartItems.innerHTML = '<p class="small-note">No items added yet.</p>';
    ...
    cartItems.innerHTML = cart.map((item, index) => `
      <div class="cart-item">
        ...
        <button type="button" class="remove-cart-item" data-index="${index}">Remove</button>
      </div>
    `).join("");
  ...
  buttons.forEach((button) => {
    button.addEventListener("click", function () {
      const card = button.closest(".commerce-product");
      ...
      const details = selects.map((select) => `${select.dataset.option}: ${select.value}`).join(" • ");

      cart.push({ product, price, quantity, details });
      renderCart();
    });
  });
});
